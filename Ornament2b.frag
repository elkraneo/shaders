...
float bandMotive1(in vec2 p, in float w) {
    ...
    m += segment(p + vec2(0.0, 0.01), p + vec2(2.0, 0.01), w);
    m += segment(p + vec2(0.0, -0.1 - 0.01), p + vec2(2.0, -0.1 - 0.01), w);
...
void main() {
    ...
    vec2 toCenter = vec2(0.0) - p;
    float angle = atan(toCenter.y, toCenter.x);
    float radius = length(toCenter) * PI;

    vec2 s = vec2(0.2, 0.2);
    float t = abs(sin(u_time / PI));

    color += bandMotive1(tile(toCenter + vec2(0.0, 0.0 + t * radius), s), width);
