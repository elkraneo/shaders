...
float geometricalElement15(vec2 p, float width) {
    float m = 0.0;

    float d = fract(PI);
    float s = width;
    
    p = tile(p, vec2(0.2, 1.0));

    p.y -= 0.05;
    p.x -= 0.1;

    m += arc(p, d, -HALF_PI / 2., PI + HALF_PI, s);
   
    p.x += 0.1;
    p.y += 0.1;
    m += arc(p, d, PI - HALF_PI / 2., PI + HALF_PI, s);
...
void main() {
    ...
    color += geometricalElement15(p, 0.01);
    p *= -1.;
    color += geometricalElement15(p, 0.005);
