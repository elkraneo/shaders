...
void main() {
    ...
    color.gb += bandMotive5Plate2(toCenter, 0.01 * cos(abs(angle - u_time * 2.)));
    toCenter.y *= -1.;
    color.gb += bandMotive5Plate2(toCenter, 0.01 * cos(abs(angle - u_time * 2.)));

    color.r += bandMotive5Plate2(toCenter, 0.05 / sin(abs(angle - u_time)));
    toCenter.y *= -1.;
    color.r += bandMotive5Plate2(toCenter, 0.05 / sin(abs(angle - u_time)));

    p += rotate(toCenter, abs(u_time / 2.0)); 

    color.gb /= sin(p.x * radius);
    color.r /= cos(p.y * radius);
    // color. *= .7;
    ...
    color /= abs(((yuv2rgb - rgb2yuv)[0]) * fract(angle));
