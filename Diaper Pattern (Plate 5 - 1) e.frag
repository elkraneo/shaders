...
void main() {
    ...
    color -= bandMotive5Plate2(toCenter, 0.01 / cos(abs(radius - u_time * 2.)));
    toCenter.y *= -1.;
    color -= bandMotive5Plate2(toCenter, 0.01 / cos(abs(angle - u_time * 2.)));

    p = rotate(toCenter, abs(u_time / 2.0)); 

    color.r /= sin(p.y + radius);
    color.g /= cos(p.x + radius);
    color.b /= .7;
